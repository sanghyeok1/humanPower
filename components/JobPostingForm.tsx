...
export default function JobPostingForm({ employerName, onSuccess }: Props) {
  // 필수 필드
  const [title, setTitle] = useState("");
  const [category, setCategory] = useState<CategorySlug>("rebar_form_concrete");
  const [startDate, setStartDate] = useState("");
  const [durationDays, setDurationDays] = useState("");
  const [shiftType, setShiftType] = useState<ShiftType>("day");
  const [workHours, setWorkHours] = useState("08:00-17:00");
  const [wageType, setWageType] = useState<WageType>("day");
  ...
  const [lodgingProvided, setLodgingProvided] = useState(false);
  const [equipmentProvided, setEquipmentProvided] = useState(false);
  const [preferredExperienceYears, setPreferredExperienceYears] = useState("");
  const [preferredCertificates, setPreferredCertificates] = useState("");
  const [preferredDriverLicense, setPreferredDriverLicense] = useState(false);
  ...
  const addressValid = useMemo(
    ...
    [roadAddress, addressDong, lat, lng]
  );

  const wageAmountValid = useMemo(() => {
    const amount = parseFloat(wageAmount);
    // 최저임금 기준 간단 체크 (2025년 기준: 시급 10,030원)
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!startDateValid) return fail("시작일은 오늘 이후여야 합니다.");
    if (!wageAmountValid) return fail("임금이 최저기준 이상이어야 합니다.");
    if (!addressValid)
      return fail("주소를 입력하고 좌표를 확인해주세요.", "addrBtn");
    if (!phoneValid)
      return fail("연락처를 정확히 입력해주세요.", "contactPhone");
    if (!phoneVerified)
      return fail("전화번호 인증이 필요합니다.", "contactPhone");
    if (!requiredPositions.trim())
      return fail("필요 인원/역할을 입력해주세요.");
    ...
    try {
      ...
      if (!res.ok || !j.ok) {
        return fail(j?.error || "공고 등록에 실패했습니다.");
      }

      setMsg("공고가 등록되었습니다!");
      setTimeout(() => {
        if (onSuccess) onSuccess(j.posting_id);
